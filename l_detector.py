...
class LDetector:
    ...
    def __init__(self, mode=None):
        """
        Inicializa el detector de Lutris
        
        Args:
            mode: Modo predefinido ('NATIVO', 'FLATPAK', 'NATIVO_DEFAULT')
        ...
        if mode:
            self._configure_by_mode(mode)
        else:
            self._detect_and_configure()
    
    def _configure_by_mode(self, mode):
        """Configura rutas basado en un modo específico"""
        if mode == "FLATPAK":
            self._configure_flatpak()
        elif mode == "NATIVO":
            self._configure_native()
        elif mode == "NATIVO_DEFAULT":
            self._configure_native_default()
        else:
            print(f"⚠️  Modo '{mode}' no reconocido. Usando detección automática...")
            self._detect_and_configure()
...
def get_lutris_paths(interactive=True, mode=None):
    """
    ...
    Args:
        ...
        mode: Modo predefinido ('NATIVO', 'FLATPAK', 'NATIVO_DEFAULT').
              Si se proporciona, ignora interactive
